...
from bftp.data.messages import Message
import openai, os
...
@router.post("/chatgpt")
async def chatgpt3_completion(
    prompt: str = Form(), temp: float = 0.5, max_tokens: int = 2000
):
    completion = create_completion(
        "text-davinci-003", prompt, temperature=temp, max_tokens=max_tokens
    )
...
async def codex_completion(
    ...
    completion = create_completion(
        "text-davinci-002", prompt, temperature=temp, max_tokens=max_tokens
    )
